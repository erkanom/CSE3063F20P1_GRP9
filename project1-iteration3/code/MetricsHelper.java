...
public class MetricsHelper {
				...
				for (Map.Entry<String, LogHistory> entry : logs.entrySet()) {
					if (entry.getValue().getUserList().contains(user)) {
						...
						if (logForUser.size() > 1) {
							doubled++;
							Log one = logForUser.get(logForUser.size()-2);
							Log two = logForUser.get(logForUser.size()-1);
							
							if(one.getLabels().containsAll(two.getLabels())) {
								decided++;
							}
						}
					}
				}
				consis += (" user " + user.getId() + " %" + (int) (decided * 1.0 / doubled * 100) + " ");
			}
	...
	public void UserMetric(ArrayList<User> users) throws FileNotFoundException {
		...
		for (int j = 0; j < users.size(); j++) {
			...
			if (user.getUserType().equals("RandomBot")||user.getUserType().equals("EvenBot")||user.getUserType().equals("HumanUser")) {
				System.out.println("\nuser: " + user.getId());
				...
				if (!Objects.isNull(logReader.GetAllDate(user))) {
					...
					if (LogDates.size() > 0) {
						...
						System.out.println("inconsistency %" + (int) ((inconsis * 1.0 / doubled) * 100));
